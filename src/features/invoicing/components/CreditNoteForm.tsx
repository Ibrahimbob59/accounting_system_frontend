...
import { makeCreditNoteSchema } from '@/features/invoicing/types/credit-notes.types'
import type {
...
interface CreditNoteFormProps {
  onSubmit: (dto: CreateCreditNoteInput) => void
  isPending: boolean
  banner: string | null
  submitLabel: string
}
...
export function CreditNoteForm({
  ...
  const postedInvoices = useSalesInvoices({ status: 'POSTED', limit: 100 })

  const schema = useMemo(() => makeCreditNoteSchema(t), [t])
  ...
  // Only the chosen customer's posted invoices can be credited.
  const invoiceOptions = (postedInvoices.data?.data ?? [])
    .filter((inv) => !customerId || inv.customerId === customerId)
    .map((inv) => ({ value: inv.id, label: inv.invoiceNo }))

  const submit = (v: CreditNoteFormValues) => {
    const num = (n: number | undefined) =>
      typeof n === 'number' && Number.isFinite(n) ? n : undefined
    const opt = (s: string | undefined) => s?.trim() || undefined
    onSubmit({
      customerId: v.customerId,
      salesInvoiceId: opt(v.salesInvoiceId),
    ...
    <form onSubmit={handleSubmit(submit)} className="space-y-6" noValidate>
      ...
      <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <SelectField
          ...
          placeholder={t('credit.noInvoice')}
        ...
        <SelectField
          id="cn-location"
          label={t('credit.restockLocation')}
...
function LineRow({
  ...
  index: number
  control: Control<CreditNoteFormValues>

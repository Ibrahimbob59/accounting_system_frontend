...
export function SalesInvoicesListPage() {
  ...
  const [status, setStatus] = useState<'all' | SalesInvoiceStatus>('all')
  const [page, setPage] = useState(1)

  const partners = useAllPartners()
  const lookupCurrency = useCurrencyLookup()

  const query: ListSalesInvoicesQuery = useMemo(
    () => ({
  ...
  const list = useSalesInvoices(query)
  ...
  const columns: ColumnDef<SalesInvoice>[] = [
    {
      ...
      cell: ({ row }) => (
        <span className="font-mono text-[13px]">{row.original.invoiceNo}</span>
      ),
    },
    {
      id: 'date',
      ...
      cell: ({ row }) => formatDate(row.original.invoiceDate, i18n.language),
    },
    {
      id: 'customer',
      enableSorting: false,
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        {canCreate && (
          <Button onClick={() => navigate('/app/sales-invoices/new')}>
      ...
      <div className="flex flex-wrap items-end gap-4">
        <SelectField
          ...
          value={status}
          onChange={(e) => {
            setStatus(e.target.value as 'all' | SalesInvoiceStatus)
            setPage(1)
          }}
          options={[
      ...
      {list.isLoading ? (
        ...
        <div className="space-y-4">
          <DataTable
            ...
            onRowClick={(row) => navigate(`/app/sales-invoices/${row.id}`)}
          />
          {meta && (
            <Pagination
              page={meta.page}
